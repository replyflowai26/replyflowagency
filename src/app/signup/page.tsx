...
export default function SignupPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        password,
        options: { data: { display_name: normalizedName }, emailRedirectTo: `${window.location.origin}/auth/confirm?next=/dashboard` },
      })
      if (error) {
  ...
  return (
    <AuthShell
      ...
      footer={<p className="text-sm text-white/40">Already have an account? <Link href="/login" className="font-medium text-cyan-300 transition hover:text-cyan-200">Sign in</Link></p>}
    >
      <form className="space-y-4" onSubmit={handleSubmit} noValidate>
        <label className="block"><span className="mb-2 block text-xs font-medium text-white/50">Your name</span><input type="text" value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="Tommy" autoComplete="name" required disabled={isSubmitting} className="h-12 w-full rounded-xl border border-white/10 bg-black/30 px-4 text-sm text-white outline-none transition placeholder:text-white/25 focus:border-cyan-300/40 focus:ring-4 focus:ring-cyan-300/[.06] disabled:opacity-60" /></label>
        ...
        <label className="block"><span className="mb-2 block text-xs font-medium text-white/50">Password</span><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 8 characters" autoComplete="new-password" minLength={8} required disabled={isSubmitting} className="h-12 w-full rounded-xl border border-white/10 bg-black/30 px-4 text-sm text-white outline-none transition placeholder:text-white/25 focus:border-cyan-300/40 focus:ring-4 focus:ring-cyan-300/[.06] disabled:opacity-60" /></label>
