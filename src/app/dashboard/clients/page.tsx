    ...
    <main className="min-h-screen bg-[#05070b] text-white">
      <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="overflow-hidden rounded-2xl border border-white/10 bg-[#090c12]/80">
          ...
          {clients.length === 0 ? <div className="px-5 py-16 text-center"><p className="text-sm text-white/50">No clients yet.</p><p className="mt-1 text-xs text-white/25">Create the first client above.</p></div> : <div className="divide-y divide-white/8">{clients.map((client) => <Link href={`/dashboard/clients/${client.id}`} key={client.id} className="grid gap-3 px-5 py-5 transition hover:bg-white/[0.025] sm:grid-cols-[1fr_auto] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-medium">{client.name}</h3><span className="rounded-full border border-white/10 px-2 py-1 text-[10px] uppercase tracking-wider text-white/45">{client.status}</span></div><p className="mt-1 text-sm text-white/40">{client.contact_name ?? "No contact"}{client.industry ? ` · ${client.industry}` : ""}</p></div><div className="text-left text-xs text-white/35 sm:text-right"><p>{client.email ?? "No email"}</p><p className="mt-1">{client.source ?? "Direct"}</p></div></Link>)}</div>}
        </section>
      </div>
    </main>
  )
