      ...
      >
        <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-white/[0.045] p-8 shadow-2xl backdrop-blur-xl">
          ...
          <form className="mt-8 space-y-4" onSubmit={handleSubmit} noValidate>
            <input type="email" name="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email address" autoComplete="email" aria-label="Email address" required disabled={isSubmitting} className="w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-white outline-none placeholder:text-white/30 focus:border-cyan-300/40" />
            <input type="password" name="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Password" autoComplete="current-password" aria-label="Password" required disabled={isSubmitting} className="w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-white outline-none placeholder:text-white/30 focus:border-cyan-300/40" />

            {(errorMessage || queryMessage) && (
              <p className="text-sm text-rose-300" role="alert">{errorMessage || queryMessage}</p>
            )}
          ...
          <div className="mt-5 flex items-center justify-between text-sm text-white/50">
            <Link className="transition hover:text-cyan-200" href="/forgot-password">Forgot password?</Link>
            <Link className="text-cyan-300 transition hover:text-cyan-200" href="/signup">Create account</Link>
          </div>
        </div>
      </motion.div>
    </main>
